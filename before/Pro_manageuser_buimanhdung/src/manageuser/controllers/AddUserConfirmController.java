...
public class AddUserConfirmController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			// Lấy ra session ở phiên hiện tại nếu có trường hợp không có
			// phiên hợp lệ nó sẽ trả về null.
			HttpSession session = request.getSession(false);
			// Kiểm tra xem có phải từ màn ADM003 qua hay không (check = true hay khong)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			TblUserInfor userInfor = (TblUserInfor) session.getAttribute("userInfor" + key);
